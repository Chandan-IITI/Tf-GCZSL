...
from tfgczsl_stg2_mst import models
from tfgczsl_stg2_mst.er_memory import ER_MEM

class TFGCZSL_STG2_MST(nn.Module):

    def __init__(self, m, n_x, n_y, n_z, nTrain, nTest, samples_per_class, total_classes, memory_batch_size, use_KD, use_der, name):
        super(TFGCZSL_STG2_MST, self).__init__()
    ...
    # function for training the tfgczsl_stg2_mst
    def train_vae(self, X_train_stm, trainData_stm, attributes_stm, trainLabels_stm, i):
        ...
        temp_dataset = TensorDataset(torch.tensor(X_train_stm),
                                     torch.tensor(trainData_stm),
                                     torch.tensor(attributes_stm),
                                     torch.tensor(trainLabels_stm))
        ...
        for epoch in range(0, self.nepoch):
            ...
            i = -1
            for iters, (X_train, trainData, attributes, trainLabels) in enumerate(temp_dataloader):
                i += 1
