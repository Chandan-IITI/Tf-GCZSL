...
from tfgczsl_stg1_mb import models
from tfgczsl_stg1_mb.er_memory import ER_MEM

class TFGCZSL_STG1_MB(nn.Module):

    def __init__(self, m, n_x, n_y, n_z, nTrain, nTest, samples_per_class, total_classes, memory_batch_size, use_KD, use_der, name):
        super(TFGCZSL_STG1_MB, self).__init__()
    ...
    # function for training the tfgczsl_stg1_mb
    def train_vae(self, X_train, trainData, attributes, trainLabels, i):
        ...
        # storing data separately for updating the replay memory
        ...
        # replay memory is used from first task onwards for training
        ...
        mean_feature, logvar_feature = None, None
        mean_att, logvar_att = None, None
        
        if len(X_train_mem) != 0:    
            ...
            loss = self.trainstep(trainData, attributes, trainData_mem, attributes_mem,
                              soft_mean_feat, soft_var_feat, soft_mean_attr, soft_var_attr,
                              mean_feature, logvar_feature, mean_att, logvar_att, self.use_KD, False)
        ...
        if self.er_mem.mem_counter >= self.er_mem.mem_size and self.epoch_done == False:
            self.epoch_done = True

            temp_dataset = TensorDataset(torch.tensor(self.er_mem.trainData_memory),
                                         torch.tensor(self.er_mem.trainLabelVectors_memory))
            temp_dataloader = DataLoader(temp_dataset, shuffle=True, batch_size=50, drop_last=False)

            for epoch in range(self.nepoch):
                ll = -1
                self.current_epoch = epoch

                for iters, (imgs, attr) in enumerate(temp_dataloader):
                    ll += 1

                    imgs, attr = imgs.cuda(), attr.cuda()
                    loss = self.trainstep(imgs, attr)
