...
from tfgczsl_stg2_mst.tfgczsl_stg2_mst_model import TFGCZSL_STG2_MST
...
# =================================================================================#
# defining the object for TFGCZSL_STG2_MST class:

tfgczsl_stg2_mst = TFGCZSL_STG2_MST(m, n_x, n_y, n_z, nTrain, nTest, samples_per_class, total_classes, memory_batch_size, use_KD, use_der, name)
...
for iter_global, (X_train, trainData, attributes, trainLabels) in enumerate(vae_train_data_loader):
    ...
    if iter_global == 0:
        X_train_stm = X_train
        trainData_stm = trainData
        attributes_stm = attributes
        trainLabels_stm = trainLabels

    elif iter_global % no_batch_stm != 0 :
        X_train_stm = torch.cat((X_train_stm, X_train), dim=0)
        trainData_stm = torch.cat((trainData_stm, trainData), dim=0)
        attributes_stm = torch.cat((attributes_stm, attributes), dim=0)
        trainLabels_stm = torch.cat((trainLabels_stm, trainLabels), dim=0)

    if iter_global%no_batch_stm == 0 and iter_global != 0 or len(X_train) < m:
        if iter_global > no_batch_stm:
            replay_TrainLabels = list(replay_Classes)

        # training the vae
        losses = tfgczsl_stg2_mst.train_vae(X_train_stm, trainData_stm, attributes_stm, trainLabels_stm, iter_global)

        replay_Classes = replay_Classes + sorted(list(set(list(trainLabels_stm.detach().numpy()))))
        replay_Classes = sorted(list(set(replay_Classes)))

        total_data_seen += len(trainLabels_stm)
...
# TEST TRAIN SPLIT
        ...
        testData_seen = testData_seen + testData_of_seen_g[0:total_data_seen].tolist()
        testLabels_seen = testLabels_seen + testLabels_of_seen_g[0:total_data_seen].tolist()
        testAttr_seen = testAttr_seen + testLabelVectors_of_seen_g[0:total_data_seen].tolist()

        # test data of unseen classes
        testLabels_unseen = testLabels_unseen + testLabels_g[0:total_data_seen].tolist()
        testData_unseen = testData_unseen + testData_g[0:total_data_seen].tolist()
        testAttr_unseen = testAttr_unseen + testLabelVectors_g[0:total_data_seen].tolist()
        ...
        def assert_cond(tfgczsl_stg2_mst, tc):
            if name == method_names[0]:
                if tc in tfgczsl_stg2_mst.er_mem.trainLabels_memory:
                    return 1
                else:
                    return 0
            else:
                if tfgczsl_stg2_mst.er_mem.classes_filled[tc] != 0:
                    return 1
                else:
                    return 0
        ...
        for tc in trainClasses:
            if tc in sorted(list(set(list(replay_TrainLabels)))) and assert_cond(tfgczsl_stg2_mst, tc):
                # generate pseudo data for previous task classes using replay memory
                labels = [tc]
                labels = np.repeat(labels, tfgczsl_stg2_mst.img_seen_samples, axis=0)
                pseudoTrainLabels_trainClasses = pseudoTrainLabels_trainClasses + labels.tolist()

                if name == method_names[0]:
                    trainData_memories = np.array(tfgczsl_stg2_mst.er_mem.trainData_memory)
                    trainLabels_memories = np.array(tfgczsl_stg2_mst.er_mem.trainLabels_memory)

                    features_of_that_class = trainData_memories[trainLabels_memories == tc, :]
                else:
                    features_of_that_class = np.array(tfgczsl_stg2_mst.er_mem.trainData_memory[tc])

                multiplier = math.ceil(np.maximum(1, tfgczsl_stg2_mst.img_seen_samples / features_of_that_class.shape[0]))
                features_of_that_class = np.repeat(features_of_that_class, multiplier, axis=0)

                if counter == 0:
                    features_to_return = torch.tensor(features_of_that_class[:tfgczsl_stg2_mst.img_seen_samples, :], dtype=torch.float)
                    counter = counter + 1
                else:
                    # print("sizes are...")
                    # print(features_to_return.size(), features_of_that_class.shape)
                    features_to_return = torch.cat((features_to_return,
                                                    torch.tensor(features_of_that_class[:tfgczsl_stg2_mst.img_seen_samples, :],
                                                                 dtype=torch.float)), dim=0)

            elif tc in sorted(list(set(list(trainLabels_stm)))):
                ...
                features_of_that_class = trainData_stm[trainLabels_stm == tc, :]
        ...
        pseudoTrainData_trainClasses = tfgczsl_stg2_mst.encode_feature(features_to_return.cuda())
        ...
        for tc in testClasses:
            labels = [tc]
            labels = np.repeat(labels, tfgczsl_stg2_mst.att_unseen_samples, axis=0)

            attrs = [ATTR[inversemap_testClasses[tc]]]
            attrs = np.repeat(attrs, tfgczsl_stg2_mst.att_unseen_samples, axis=0)
        ...
        if len(testClasses) != 0:
            ...
            pseudoTrainData_testClasses = tfgczsl_stg2_mst.encode_attr(torch.tensor(pseudoTrainAttr_testClasses, dtype=torch.float).cuda())
        ...
        testData_seen = tfgczsl_stg2_mst.encode_test_data(torch.tensor(testData_seen, dtype=torch.float).cuda())
        ...
        # encode the test data using the feature attribute (no sampling is done)
        testData_unseen = tfgczsl_stg2_mst.encode_test_data(torch.tensor(testData_unseen, dtype=torch.float).cuda())
        ...
        print("remaining data is...", len(trainLabels_g[total_data_seen-1:len(trainLabels_g)]))
...
# saving data:
with open('results_tfgczsl_stg2_mst_reservoir_stm.csv', 'a') as file:
